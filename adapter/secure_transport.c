...
long secure_https_post_json_unauth(const char *url, const char *body,
    ...
    if (status_out) *status_out = status;
    if (code != CURLE_OK) {
        set_error("interaction callback", code);
        return -(long)code;
    }
    return (long)output.length;
}
...
long secure_https_delete(const char *url, const char *authorization,
    ...
    if (status_out) *status_out = status;
    if (code != CURLE_OK) {
        set_error("https delete", code);
        return -(long)code;
    }
    return (long)output.length;
}

long secure_https_delete_with_header(const char *url, const char *authorization,
    ...
    if (extra_header && extra_header[0] != '\0') {
        next = curl_slist_append(headers, extra_header);
        if (!next) {
            curl_slist_free_all(headers);
            curl_easy_cleanup(easy);
            return -ENOMEM;
...
long secure_https_get(const char *url, const char *authorization,
                      char *response, size_t response_capacity,
                      long *status_out) {
    CURL *easy = curl_easy_init();
    struct curl_slist *headers = NULL;
    ...
    if (status_out) *status_out = status;
    if (code != CURLE_OK) {
        set_error("https get", code);
        return -(long)code;
    }
    return (long)output.length;
}

long secure_https_patch_json(const char *url, const char *authorization,
                             const char *body, size_t body_length,
                             char *response, size_t response_capacity,
                             long *status_out) {
    CURL *easy = curl_easy_init();
    ...
    CURLcode code;
    long status = 0;
    if (!easy || !authorization || !body || !response || response_capacity < 2) return -EINVAL;
    ...
    if (status_out) *status_out = status;
    if (code != CURLE_OK) {
        set_error("https patch json", code);
        return -(long)code;
    }
    return (long)output.length;
}

long secure_https_put_json(const char *url, const char *authorization,
                           const char *body, size_t body_length,
                           char *response, size_t response_capacity,
                           long *status_out) {
    CURL *easy = curl_easy_init();
    ...
    if (status_out) *status_out = status;
    if (code != CURLE_OK) {
        set_error("https put json", code);
        return -(long)code;
    }
    return (long)output.length;
}

long secure_https_put_json_with_header(const char *url, const char *authorization,
                                       const char *extra_header,
                                       const char *body, size_t body_length,
                                       char *response, size_t response_capacity,
                                       long *status_out) {
...
long secure_https_put_empty(const char *url, const char *authorization,
                            char *response, size_t response_capacity,
                            long *status_out) {
    CURL *easy = curl_easy_init();
    struct curl_slist *headers = NULL;
